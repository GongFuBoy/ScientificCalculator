...
本项目按“需求梳理 → 设计 → 编码 → 自动化测试 → curl 验收 → 打包”全流程使用 AI 辅助，并由本人逐项校验和修正。
...
| 自动化测试 | 生成 JUnit 5、MockMvc 和 Spring 上下文测试矩阵 | 先观察缺失类导致的 RED，再补齐实现；增加历史、415/422/500、清空和失败原子性验收 | `./mvnw clean verify` |
| curl 验收 | 基于需求、设计和接口实现整理健康检查、计算、历史、参数、语法、领域错误和资源限制案例 | 按顺序执行全部案例；复核状态码、错误码、计算结果、失败不写历史及进程重启语义 | [curl 测试案例](testing/CURL_TEST_CASES.md)、[测试与验收证据](testing/TEST_EVIDENCE.md) |
